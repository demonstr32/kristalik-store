...
class AuthService:
    ...
    async def register_user(self, user_create: UserCreateSchema)-> UserResponseSchema:
        user_orm = await self.user_repository.get_user_by_email(email=user_create.email)
        if user_orm:
            raise UserAlreadyExists(f"Пользователь с email: {user_create.email} уже существует")
        hashed_password = hash_password(user_create.password)
        user_orm = await self.user_repository.create_user(email=user_create.email,hashed_password=hashed_password)
        await self.db.commit()
        await self.db.refresh(user_orm)
        return UserResponseSchema.model_validate(user_orm)
    
    async def login_user(self, user_log: UserLoginSchema)->tuple[UserResponseSchema,str]:
        user_orm = await self.user_repository.get_user_by_email(email=user_log.email)
        if not user_orm:
    ...
    async def update_user(self, user_id: str, user_update: UserUpdateSchema)-> UserResponseSchema:
    ...
    async def delete_user(self, user_id: str)->None:
        user_for_del = await self.user_repository.get_user_by_id(user_id=user_id)
        if not user_for_del:
            raise UserNotFound(f"Пользователь с id: {user_id} не найден")
        await self.user_repository.delete_user(user_for_del)
        await self.db.commit()
