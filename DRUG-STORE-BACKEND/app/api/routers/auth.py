...
async def register_user(
    ...
    try:
        return await auth_service.register_user(user_create=payload)
...
async def login_user(
    ...
    try:
        user, token = await auth_service.login_user(user_log=payload)
...
async def update_user(
    ...
    try:
        return await auth_service.update_user(user_id=current_user.id, user_update=payload)
    except UserNotFound as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
