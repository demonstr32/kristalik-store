...
from app.api.dependencies import get_prod_service
...
def create_product(payload: ProductCreateSchema, prod_service: ProductService = Depends(get_prod_service))->ProductResponseSchema:
    
    return prod_service.create_prod(prod_create=payload)
@router.patch("/{prod_id}")
def update_product(prod_id: int,payload: ProductUpdateSchema, prod_service: ProductService = Depends(get_prod_service))->ProductResponseSchema:
    try:
...
def delete_product(prod_id: int,prod_service: ProductService = Depends(get_prod_service))->None:
    try:
